...
import com.example.demoproj.AppStore;
...
//    public static String serverUrl = "http://202.194.14.120:9090";
    ...
    public static String login(com.example.demoproj.request.LoginRequest request){
        if(isLocal) {
            return com.example.demoproj.request.SQLiteJDBC.getInstance().login(request.getUsername(),request.getPassword());
        }else {
            ...
            try {
                ...
                if (response.statusCode() == 200) {
                    com.example.demoproj.request.JwtResponse jwt = gson.fromJson(response.body(), com.example.demoproj.request.JwtResponse.class);
                    AppStore.setJwt(jwt);
                    return null;
    ...
    public static DataResponse request(String url,DataRequest request){
        if(isLocal) {
            ...
            try {
                Method method = com.example.demoproj.request.SQLiteJDBC.class.getMethod(methodName, DataRequest.class);
                return (DataResponse)method.invoke(com.example.demoproj.request.SQLiteJDBC.getInstance(), request);
            }catch(Exception e) {
                e.printStackTrace();
            }
        }else {
     ...
     */
    public static com.example.demoproj.request.MyTreeNode requestTreeNode(String url, DataRequest request){
        HttpRequest httpRequest = HttpRequest.newBuilder()
                .uri(URI.create(serverUrl + url))
                .POST(HttpRequest.BodyPublishers.ofString(gson.toJson(request)))
                .headers("Content-Type", "application/json")
        ...
        try {
            HttpResponse<String>  response = client.send(httpRequest, HttpResponse.BodyHandlers.ofString());
            if(response.statusCode() == 200) {
                return gson.fromJson(response.body(), com.example.demoproj.request.MyTreeNode.class);
            }
        } catch (IOException e) {
            e.printStackTrace();
        } catch (InterruptedException e) {
    ...
    public static List<com.example.demoproj.request.MyTreeNode> requestTreeNodeList(String url, DataRequest request){
        HttpRequest httpRequest = HttpRequest.newBuilder()
                .uri(URI.create(serverUrl + url))
                .POST(HttpRequest.BodyPublishers.ofString(gson.toJson(request)))
                .headers("Content-Type", "application/json")
        ...
        try {
            ...
            if(response.statusCode() == 200) {
                ...
                List<com.example.demoproj.request.MyTreeNode> rList = new ArrayList<>();
                for(int i = 0; i < list.size();i++) {
                    rList.add(new com.example.demoproj.request.MyTreeNode((Map)list.get(i)));
                }
                return rList;
            }
        } catch (IOException e) {
     ...
     */
    public static List<com.example.demoproj.request.OptionItem> requestOptionItemList(String url, DataRequest request){
        HttpRequest httpRequest = HttpRequest.newBuilder()
                .uri(URI.create(serverUrl + url))
                .POST(HttpRequest.BodyPublishers.ofString(gson.toJson(request)))
                .headers("Content-Type", "application/json")
        ...
        try {
            HttpResponse<String>  response = client.send(httpRequest, HttpResponse.BodyHandlers.ofString());
            if(response.statusCode() == 200) {
                com.example.demoproj.request.OptionItemList o = gson.fromJson(response.body(), com.example.demoproj.request.OptionItemList.class);
                if(o != null)
     ...
     * @return List<OptionItem> 返回后台返回数据
     */
    public static  List<com.example.demoproj.request.OptionItem> getDictionaryOptionItemList(String code) {
    ...
    public static int uploadHtmlString(String html)  {
            ...
            return CommomMethod.getIntegerFromObject(res.getData());
    }


    public static List<Course> requestCourseList(){
        HttpRequest httpRequest = HttpRequest.newBuilder()
                .uri(URI.create(serverUrl + "/api/course/getCourseListSerializable"))
                .POST(HttpRequest.BodyPublishers.ofString(gson.toJson(new DataRequest())))
                .headers("Content-Type", "application/json")
                .headers("Authorization", "Bearer "+AppStore.getJwt().getAccessToken())
                .build();
        HttpClient client = HttpClient.newHttpClient();
        try {
            ...
            if(response.statusCode() == 200) {
                ...
                List<Course> list = (List<Course>)in.readObject();
                in.close();
                return list;
            }
        } catch (Exception e) {
